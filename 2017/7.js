...
const nodes = `wdysq (135) -> sxldvex, wiasj
...
inelfwo (91)
vgkhk (76)
cckrzh (6841) -> hdinwud, dqzkic, vrxkr`.split`
`.map(s => {
	const [a, b] = s.split(' -> ')
	const [name, weight] = a.slice(0, -1).split(' (')
	console.log(weight)
	const children = b ? b.split(', ') : []
	return {
		name,
		weight: +weight,
		children,
		get netWeight(){
		    ...
		    for(const child of this.children) weight += child.netWeight
		    delete this.netWeight
		    return this.netWeight = weight
		},
		get isBalanced(){
		    this.childWeights = this.children.map(
		        c => c.netWeight
		    )
		    return !this.children.length || this.children.every(
		        c => c.netWeight === this.children[0].netWeight
		    )
		}
	}
})
...
for(const node of nodes){
	node.children = node.children.map(name => nodes.find(node => node.name === name))
...
// Part 2
const unbalancedNodes = nodes.filter(n => !n.isBalanced)
...
[ { name: 'vgzejbd',
    ...
    netWeight: [Getter],
    isBalanced: [Getter],
    childWeights: [ 159240, 159246, 159240, 159240, 159240, 159240 ] },
  { name: 'vwkkml',
    weight: 148790,
    ...
    parent: 
     { name: 'vgzejbd',
       ...
       childWeights: [Array] },
    netWeight: 159246,
    childWeights: [ 2090, 2090, 2090, 2090, 2096 ] } ]
*/
const nodeWithWrongWeight = nodes.find(n => n.name === 'vwkkml').children[4]
console.log(nodeWithWrongWeight.weight - 6)