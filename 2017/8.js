...
for(const instruction of instructions){
    const [var1, sign, dx, _, var2, compare, right] = instruction.split(' ')
    vars[var1] = vars[var1] || 0
    vars[var2] = vars[var2] || 0
    
    const left = vars[var2]
    if(eval(`${left} ${compare} ${right}`)){
        vars[var1] += sign === 'inc' ? + +dx : - +dx
        allTimeMax = Math.max(allTimeMax, vars[var1])
    }
}
console.log(vars)

// Part 1
console.log(Math.max(...Object.values(vars)))
